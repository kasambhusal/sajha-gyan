...
export default function RandomPracticePage() {
  ...
  const initializeRandomTest = () => {
    ...
    const availableQuestions = allQuestions.filter((q) => {
      const attemptedIds = getAttemptedQuestions(q.subjectId, q.subtopicId)
      return !attemptedIds.includes(q.id)
    })
    ...
    // Shuffle and take 15 random questions
    const shuffledQuestions = [...availableQuestions].sort(() => Math.random() - 0.5)
    const testQuestions = shuffledQuestions.slice(0, Math.min(15, shuffledQuestions.length))
  ...
  const handleAnswerSubmit = () => {
    ...
    if (currentQuestion.type === "mcq") {
      ...
      correct = writtenAnswer.trim().length > 10
    }
    ...
    // Update progress
    updateUserProgress(currentQuestion.subjectId, currentQuestion.subtopicId, currentQuestion.id, correct, timeTaken)
    ...
    const result = {
      ...
      subject: currentQuestion.subjectTitle,
      subtopic: currentQuestion.subtopicTitle,
  ...
  if (testCompleted) {
    const finalResults = [...testResults, { isCorrect, timeTaken: Date.now() - questionStartTime }]
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="w-full max-w-2xl">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl">Random Practice Completed!</CardTitle>
            <CardDescription>You tackled questions from multiple subjects</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="flex gap-4">
              ...
              <Button variant="outline" onClick={() => router.push("/practice/random")} className="flex-1">
                Try Again
              </Button>
            </div>
          </CardContent>
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl font-bold">Random Practice</h1>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <Badge variant="outline">{currentQuestion.subjectTitle}</Badge>
                  <Badge variant="secondary">{currentQuestion.subtopicTitle}</Badge>
                </div>
                <div className="flex items-center space-x-2">
                  <Badge
                    variant={
                  ...
                  <Badge variant="outline">{currentQuestion.type === "mcq" ? "MCQ" : "Written"}</Badge>
                </div>
              </div>
